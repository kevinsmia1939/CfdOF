...
def _sanitiseRegionName(name):
    """Return a valid OpenFOAM / TemplateBuilder region name (alphanumeric + underscore, no spaces)."""
...
class CfdCaseWriterFoam:
    ...
    def processFluidProperties(self):
        ...
        for solid_obj in self.solid_material_objs:
            ...
            region_name = _sanitiseRegionName(getattr(solid_obj, 'RegionName', '') or solid_obj.Label)
        ...
        for material_obj in self.material_objs:
            ...
            explicit_region = _sanitiseRegionName(getattr(material_obj, 'RegionName', ''))
            if explicit_region:
                ...
                region_name = _sanitiseRegionName(internal_zones[0] if internal_zones else material_obj.Label)
            else:
                region_name = _sanitiseRegionName(material_obj.Label)
