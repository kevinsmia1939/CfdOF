    ...
    def processMultiRegionBoundaries(self):
        ...
        def touched_regions_for_boundary(bc_obj):
            ...
            if explicit_region:
                if explicit_region in settings['multiRegionSolidNames']:
                    return [(explicit_region, 'solid')]
                return [(explicit_region, 'fluid')]
        ...
        for bc_obj in self.bc_group:
            ...
            if explicit_region and not is_region_interface:
                matched = add_boundary_to_region_maps(
                    bc_obj,
                    [(explicit_region,
                      'solid' if explicit_region in settings['multiRegionSolidNames'] else 'fluid')])
            else:
                matched = False
        ...
        for entry in region_coupled_bcs:
            if not entry['regions']:
                raise ValueError(
                    ...
                    "Use a BooleanFragments result and select the shared/generated interface face, "
                    "or set RegionName explicitly.".format(entry['obj'].Label))
